...
public class ChangelogTask extends DefaultTask
{
    ...
    private List<Map<String, Object>> getBuildInfo()
    {
        ...
        try
        {
            ...
            data = cleanJson(data, "{}"); // Empty entries, just for sanities
                                          // sake
            
            List<Map<String, Object>> json = (List<Map<String, Object>>) new Gson().fromJson(data, Map.class).get(
                    "allBuilds");
            ...
            while (bitr.hasNext())
            {
                ...
                if (actions.size() == 0)
                {
                    build.put("version", versioned ? ((Double) build.get("number")).intValue() : getProject()
                            .getVersion());
                    versioned = true;
                }
                else
                ...
                for (Map<String, Object> e : (List<Map<String, Object>>) ((Map<String, Object>) build.get("changeSet"))
                        .get("items"))
                {
    ...
    private void getLatestBuild(List<Map<String, Object>> builds)
    {
        ...
        try
        {
            ...
            Map<String, Object> build = new Gson().fromJson(data, Map.class);
            if (build.get("number").equals(ver))
            {
                return;
            }
            build.put("version", versioned ? "Build " + ((Double) build.get("number")).intValue() : getProject()
                    .getVersion());
